...
local Command = {commands={},autoplay = {}}
...
function helpFunction(data)
	...
	if args[1] then
		...
		embed = {
			...
			title = (Data.title or bot.user.username)..(Data.ownerName and " by "..Data.ownerName or ""),
...
function Command:newMsg(message)
	...
	local function isAllowed(options,joined)
		if Data.owner then
			if type(Data.owner) == "table" then
				for i,v in pairs(Data.owner) do
					if v == author.id then
						return true
					end
				end
			else
				if Data.owner == author.id then 
					return true
				end
			end
		end
		...
		local usageText = options.usage ~= "" and options.usage and "Correct usage: "..options.usage or ""
		...
		if argsRequired then
			if #joined == 0 then
				channel:sendMessage{content="Arguments are required for this command. "..usageText}
				message:addReaction("⛔")
				return
			end
		end
		...
		if deleteCommand then message:delete() end
	...
	local function execute(beginning,isSubCommand)
		...
		local success,err = pcall(function()
			local data = {autoplay = Command.autoplay,command = command,message=message,joined=args,args=joined,bot = message.bot,me = (message.guild and message.guild.me),channel = channel,guild = message.guild,author = message.author,member = message.member}
...
function Command:sendEmbed(data,content,embed)
	...
	if embed and data.me then
		embed.color = (data.me.color.value ~= 0 and data.me.color.value) or "15105570"
	elseif not data.guild and embed then
